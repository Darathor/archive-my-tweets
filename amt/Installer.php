...
class Installer
{
	...
	public function step1()
	{
		...
		if (isset($_POST['installer']))
		{
			...
			if (!$installProblem)
			{
				...
				{
					$dsn =
						"mysql:host=" . $this->data['form']['databaseHost'] . ";dbname=" . $this->data['form']['databaseDatabase'] . ";charset=utf8";
	...
	public function validateFormData($data)
	{
		...
		// twitter username
		if (!preg_match("/^[a-zA-Z0-9_]+$/", $data['twitterUsername']) || strlen($data['twitterUsername']) < 1
			|| strlen($data['twitterUsername']) > 15
		)
		{
		...
		$requiredFields =
			['twitterName', 'twitterUsername', 'consumerKey', 'consumerSecret', 'oauthToken', 'oauthSecret', 'baseUrl', 'timezone', 'cronKey',
	...
	public function writeConfig($directory, $data)
	{
		...
		$config .= "define('TWITTER_ID',              " . (int)$data['twitterId'] . ");\n";
	...
	public function verifyRequirements()
	{
		...
		{
			$problems['errors'][] = 'PHP 5.5.0 or greater is required. Your installed version is ' . phpversion() . '.';
		}
